...
class Inovarti_Onestepcheckout_AjaxController extends Mage_Checkout_Controller_Action {
    ...
    public function busca_cepAction() {
       if ($this->getRequest()->getPost()) {
        ...
        /*
        * SOLUÇÃO PARA FUNCONAR REGRA DE PREÇO
        */
        $result  = array('localidade' => '', 'uf' => '', 'cep' => '' );
        ...
        curl_setopt_array($ch, array
            (
                CURLOPT_URL => "http://www.buscacep.correios.com.br/sistemas/buscacep/resultadoBuscaCepEndereco.cfm?t",
                CURLOPT_POST => TRUE,
        ...
        $info = $matches[1];
        if (count($info) == 4) {
            $baseCidadeUf = explode('/', $info[2]);
            $result = array(
                'localidade' => html_entity_decode(utf8_encode($baseCidadeUf[0])),
                'uf' => html_entity_decode(utf8_encode($baseCidadeUf[1])),
                'cep' => utf8_encode(str_replace('-', '', $info[3])));
            if ($info[1] != '&nbsp;') {
                $result['bairro'] = html_entity_decode(utf8_encode($info[1]));
            }

            if ($info[0] != '&nbsp;') {
                $result['logradouro'] = html_entity_decode(utf8_encode(trim($info[0])));
            }
        } elseif (count($info) > 4) {
            $newCep = substr($cep, 0, 5) . '-' . substr($cep, 5);
            $keyCep = array_search($newCep, $info);

            $baseCidadeUf = explode('/', $info[$keyCep - 1]);

            $result = array(
                'localidade' => html_entity_decode(utf8_encode($baseCidadeUf[0])),
                'uf' => html_entity_decode(utf8_encode($baseCidadeUf[1])),
                'cep' => utf8_encode(str_replace('-', '', $info[$keyCep])));

            if ($info[1] != '&nbsp;') {
                $result['bairro'] = html_entity_decode(utf8_encode($info[$keyCep - 2]));
            }

            if ($info[0] != '&nbsp;') {
                $result['logradouro'] = html_entity_decode(utf8_encode($info[$keyCep - 3]));
            }
        } else {
            $result = array();
        }
