function wpShowMenuPopup(objMenu, event, popupId)
{
    ...
    if (!objMenu.hasClassName('active')) {
        wpCustommenuTimerShow[popupId] = setTimeout(function() {
            ...
            jQuery('#' + popupId).stop(true, true).slideDown();
            ...
            if (!wpIsChildOf(popup, wpCurrentMouseTarget) && popup != wpCurrentMouseTarget) {
                if (objMenu.hasClassName('active')) {
                    wpCustommenuTimerHide[popupId] = setTimeout(function() {
                        ...
                        jQuery('#' + popupId).stop(true, true).slideUp();
...
{
    if (j$("#menu-a").hasClass("active")) {
	j$("#menu-a").removeClass('active');
	j$("#menu-content").slideUp(400);
    }
    else {
	j$("#menu-a").addClass('active');	
	j$("#menu-content").slideDown(400);
    }
}
